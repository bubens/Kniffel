module.exports = function (grunt) {
	...
	var src = require("./Gruntfiles/files.src.js"),
		...
		ftpconfig = require("./config/ftpconfig.js"),
		...
		config = {
			...
			"sftp-deploy": {
				test: {
					auth: {
						host: ftpconfig.test.server,
						port: ftpconfig.test.port,
						authKey: ftpconfig.test.authKey,
						secure: true
					},
					src: "kniffel-v<%= pkg.version %>/",
					dest: ftpconfig.test.dest,
					exclusions: ["config"],
					silent: false
				},
				release: {
					auth: {
						host: ftpconfig.release.server,
						port: ftpconfig.release.port,
						authKey: ftpconfig.release.authKey,
						secure: true
					},
					src: "kniffel-v<%= pkg.version %>/",
					dest: ftpconfig.release.dest,
					exclusions: ["config"]
				}
			}
	...
	grunt.registerTask("test-deploy", "Deploy for testing...", ["sftp-deploy:test"]);
	grunt.registerTask("release", "Deploy release...", ["sftp-deploy:release"]);
